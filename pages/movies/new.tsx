...
const NewReleases = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/movies/new-releases`);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {sortedMovies.map((movie) => (
          <Card key={movie.id} className="relative shadow-lg">
            <Image src={movie.poster} alt={movie.title} width={500} height={300} className="w-full h-60 object-cover rounded-t-lg" />
